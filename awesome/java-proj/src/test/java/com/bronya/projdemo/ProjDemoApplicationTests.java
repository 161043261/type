...
class ProjDemoApplicationTests {
  ...
  public void testJwt() {
    ...
    String jwtString =
        Jwts.builder() // get a JwtBuilder
            .header()
            .keyId("bronya")
            .and()
            .claims(claimsMap) // payload
    ...
    // parse jwtString
    Claims claims =
        Jwts.parser() // get a JwtParserBuilder
            .verifyWith(decoded)
            .build() // get a thread-safe JwtParser
            .parseSignedClaims(jwtString)
            .getPayload(); // parse jwtString
  ...
  public void testThreadLocal() {
    ...
    new Thread(
            () -> {
              ...
              System.out.println(
                  "K = " + Thread.currentThread().getName() + ", V = " + threadLocal.get());
            },
            "ThreadA")
        .start(); // K = ThreadA, V = ValueA
    new Thread(
            () -> {
